...
public interface TicketBillRowRepository extends JpaRepository<TicketBillRow, Long> {
	...
	@Query(value = "WITH report_table AS (" +
				...
				")" +
				"SELECT" +
				"	bill_id AS Id, serial_no AS SerialNo, :ticketId AS TicketId, :ticketName AS TicketName, visitor_id AS VisitorId, visitor_name AS VisitorName, SUM(total_sum) AS TotalSum, COUNT(bill_id) AS Persons, SUM(price) AS Price, ticket_serial AS TicketSerial, cancelled_status AS CancelledStatus" + "\n" +
				"FROM" +
				"	report_table" + "\n" +
				"GROUP BY" +
				"	bill_id, visitor_id, visitor_name, persons, ticket_serial, cancelled_status", nativeQuery = true)
	public List<TicketReportTable> getOverallTicketSales(long ticketId, String ticketName);
	...
	@Query(value = "WITH report_table AS (" +
				...
				"GROUP BY" +
				"	bill_id, visitor_id, visitor_name, persons, ticket_serial, cancelled_status", nativeQuery = true)
	public List<TicketReportTable> getOverallTicketSales(long ticketId, String ticketName, Timestamp startDateTime, Timestamp endDateTime);
